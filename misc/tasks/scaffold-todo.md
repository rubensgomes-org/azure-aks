...
`azure-aks` was scaffolded from `spring-blueprint` as build/CI/release machinery
only. This file records what was deliberately left outstanding so the scaffold
could be handed over green.

## Outstanding

- [ ] Add the Azure dependency — the Azure Kubernetes Service deployment path — to `app/build.gradle.kts`,
...
- [ ] Build the real endpoint that demonstrates Azure Kubernetes Service.
...
- [ ] Create the SonarCloud project `rubensgomes-org_azure-aks` in the
