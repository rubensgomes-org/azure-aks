...
How the Gradle build for `azure-aks` is put together, what each task
...
> **Note** — `logging.level.com.rubensgomes.azure.aks` is set to `trace` in the
...
```bash
kill $(pgrep -f com.rubensgomes.azure.aks.App)
```
...
```bash
./gradlew bootJar
java -jar app/build/libs/azure-aks-<version>.jar
```
...
```kotlin
excludes = listOf("com.rubensgomes.azure.aks.config.*")
```
...
| Archive                                  | Contents                                                 |
|------------------------------------------|----------------------------------------------------------|
| `azure-aks-<version>.jar`         | executable Spring Boot jar (layered, for Docker caching) |
| `azure-aks-<version>-plain.jar`   | library jar, classes only                                |
| `azure-aks-<version>-sources.jar` | sources                                                  |
| `azure-aks-<version>-javadoc.jar` | Javadoc                                                  |
...
| `sonar.projectKey`       | `rubensgomes-org_azure-aks` |
| `sonar.projectName`      | `azure-aks`                 |
...
there is no `azure-aks-spring-boot.jar` for the Dockerfile and no `.env`.
